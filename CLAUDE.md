...
- **Display** (~600 lines in `displayResults()`): thread rendering, compact/detailed views, stats, code context links
...
- **Validation**: Add unit-test whenever possible and relevant, run unit-test suite. Run tests with: `~/.nvm/versions/node/v20.20.0/bin/npm test` — `nvm` is not available in non-interactive shells, so use the full path. (`vitest.config.mjs` requires Node 20; the system default is Node 18 which fails.)
- **DOM-based tests**: The default vitest environment is `node`. Tests that need DOM APIs (e.g. testing scroller layout via `container.style.height`, or any code that calls `document.createElement`) require installing `happy-dom` as a dev dependency and adding `// @vitest-environment happy-dom` at the top of the test file. The scroller's `mount()` / `recalcLayout()` / `getTotalHeight()` are all public and testable this way. `getBoundingClientRect` returns zeros in happy-dom — this is fine: `_codeRowHeight` falls back to `DEFAULT_CODE_HEIGHT = 19` and thread heights use the estimate formula. To force the large-file code path (virtual scroll, `position:absolute` layout) you need 500+ rows (`SMALL_FILE_THRESHOLD = 500`); `recalcLayoutInternal` early-returns for small files.
